...
<html>
	<head>
		...
		<!--
		contextmenu.putMenus("menu", [["查看指示书", "view.gif", "doShowZs()"],	["打印钢卷缺陷", "edit.gif", "doDyZss()"]]);
		...
		function selPrint() {		
			...
			var postContent = coco.parseParam("ids", idStr);
			var ajax = new Cocoajax();
			ajax.callback = function() {
				if(this.code <= 0){
					coco.alert(this.msg);
				...
				document.getElementById("PrintFrame").src = path + "/sino/dy/jbqxb.do?type=0&nos="+idStr;
				cocopage.refresh();
			};
			ajax.post("updateZsQxPrint.do", postContent);
		}
		...
		function doDyZss() {
			...
			ajax.callback = function() {
				...
				document.getElementById("PrintFrame").src = path + "/sino/dy/jbqxb.do?type=0&nos="+zsno;
				cocopage.refresh();
			};
...
<body>
<ui:page title="指示书缺陷打印 ">
   <f:page action="indexQxDy.do" file="listQxDy.jsp">
	...
	<table width="96%" align="center" style="margin: 10px 0;" class="form">
		...
		<tr>
			...
			<td><ui:select   name="zsqx"  list="'0':'未打印','1':'已打印'" /></td>
		</tr>
		<tr>
	...
	<div class="submit">
		<input type="button" class="button" value="查询" onclick="doQuery(this.form);" />	
		<input type="button" class="button" onclick="selPrint()" value="打印钢卷缺陷" accesskey="c">
	</div>
  </f:page>
</ui:page>
<ui:panel id="View" title="指示书详细信息" popup="true" display="false" closable="true" >
<DIV id="ViewArea" ></DIV></ui:panel>
