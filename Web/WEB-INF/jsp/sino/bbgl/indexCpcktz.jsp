...
<head>
		...
		function saveTz(el) {
			...
			var form = el.form;
			var cplb = form.elements["cplb"].value;
			tableForm.submit("saveCptz.do?cplb="+encodeURIComponent(cplb));
		}
		//-->
		</SCRIPT>
	</head>
	<body>
	<ui:page title="成品仓库台帐管理">
		<f:page action="indexCpcktz.do" file="listJzpcktz.jsp">
			<table width="100%" class="form">
				...
				<tr>
					<th style="text-align: left;">成品类别</th>
					<td><ui:select name="cplb" list="${cplb }" onchange="cocopage.query();" /></td>
					...
					<td><ui:select name="chou" sql="#select KEY_,VALUE_ from COCO_CODE_ITEM where CODE_='SPBH'" prop="nn: 1;" /></td>
				</tr>
			</table>
			<div class="submit">
			<input type="button" class="button" value="查 询" onclick="cocopage.query();" />
			<input type="button" class="button" value="保 存" onclick="saveTz(this);" />
			</div>	
		</f:page>
	</ui:page>
	</body>
