...
<html>
	<head>
		...
		function outExcel() {
			...
			if(window.self == window.top) wo.LoadOriginalFile("./outEtlSyExcel.do?"+content, "xls");
			else wo.LoadOriginalFile("<%=request.getContextPath()%>/sino/etl/sj/outEtlSyExcel.do?"+content, "xls");
	...
	<div id="QueryArea">
		...
		<table align="center" class="form" width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
				<th>日期：</th>
				<td><ui:date name="etsd" required="true" value="{date}"/></td>
				<td><input type="button" class="button" value="查询(Q)" accesskey="q" onclick="outExcel();" /></td>
			</tr>
		</table>
		</form>
		</div>
