...
public class MoneyFormat {
	...
	public static MoneyFormat getInstance() {
		if (formatter == null) formatter = new MoneyFormat();
		return formatter;
	}
	...
	private String convertToChineseNumber(String moneyStr) {
		...
		if (cMoneyStringBuffer.lastIndexOf(ZERO) == cMoneyStringBuffer.length() - 1) cMoneyStringBuffer.delete(cMoneyStringBuffer.length() - 1, cMoneyStringBuffer.length());
	...
	private String addUnitsToChineseMoneyString(String moneyStr) {
		...
		cMoneyStringBuffer.replace(cMoneyStringBuffer.indexOf("零分"), cMoneyStringBuffer.length(), "整");
			...
			if (cMoneyStringBuffer.indexOf("零角") != -1) cMoneyStringBuffer.delete(cMoneyStringBuffer.indexOf("零角"), cMoneyStringBuffer.indexOf("零角") + 2);
			// tmpBuffer.append("整");
		}
		result = cMoneyStringBuffer.toString();
		return result;
	...
	private void checkPrecision(String moneyStr) {
		...
		if (fractionDigits > 2) throw new RuntimeException("金额" + moneyStr
				+ "的小数位多于两位。"); // 精度不能比分低
	}
	...
	public static void main(String args[]) {
		 ...
		 * if('.'==s.charAt(j)){ continue; } System.out.print(s.charAt(j));
		 */
	}
