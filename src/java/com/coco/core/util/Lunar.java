...
public class Lunar {
	 ...
	 * 短时间格式
	 */
	public final static int FORMAT_MM_DD = 1;
	...
	private final static String[] TWELVE_ANIMALS = new String[] { "鼠", "牛",
			"虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪" };
	...
	private final static String CHINESE_UNIT[] = { "初", "十", "廿", "卅" };
	...
	public Lunar(Calendar cal) {
		...
		Date baseDate = null;
		try {
			baseDate = DATE_FORMAT.parse("1900年1月31日");
		}
	...
	private int leapDays(int y) {
		if (leapMonth(y) != 0) {
			if ((LUNAR_INFO[y - 1900] & 0x10000) != 0) return 30;
			else return 29;
		}
		else return 0;
	}
	...
	public String animalsYear() {
		return TWELVE_ANIMALS[(year - 4) % 12];
	}
	...
	public String getChinaDayString(int day) {
		...
		if (day > 30) return "";
		if (day == 10) return "初十";
		else return CHINESE_UNIT[day / 10] + CHINESE_NUMBER[n];
	}

	/**
	 * 重载toString方法，对象直接输出农历格式
	 */
	@Override
	...
	public String format(int type) {
		if (type == FORMAT_MM_DD) {
			return CHINESE_NUMBER[month - 1] + "月" + getChinaDayString(day);
		}
		return toString();
	}
}