...
import com.quanta.sino.ss.vo.SsBlVO.DetailItem;
 ...
 * @version 1.0
 */
public class BlmxExcelDataExecuter implements ExcelDataExecuter<SsBlVO> {
	...
	public void execute(HSSFSheet sheet, SsBlVO data) {
		if (data.getDetails() == null || data.getDetails().length == 0) {
			return;
		}
		HSSFRow row;
		int rowIndex = 4;
		Integer cellnum;
		Integer zshu;
		for (DetailItem vo : data.getDetails()) {
			row = sheet.createRow((rowIndex++));
			ExcelUtils.setCellValue(row.createCell(0), vo.getTimeLabel());
			ExcelUtils.setCellValue(row.createCell(1), vo.getJbno());
			ExcelUtils.setCellValue(row.createCell(2), vo.getZrzl() > 0 ? vo.getZrzl()
					: "");
			ExcelUtils.setCellValue(row.createCell(3), vo.getDanz());
